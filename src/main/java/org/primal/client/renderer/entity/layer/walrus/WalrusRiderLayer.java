...
public class WalrusRiderLayer<T extends WalrusEntity, M extends WalrusModel<T>> extends RenderLayer<T, M> {
    ...
    @Override
    public void render(PoseStack poseStack, MultiBufferSource bufferSource, int packedLight, T entity, float pLimbSwing, float pLimbSwingAmount, float partialTick, float pAgeInTicks, float pNetHeadYaw, float pHeadPitch) {
        if (entity.isVehicle()) {
